...
async function applyGraphVisualState() {
    ...
    try {
        ...
        const theme = 'dark';
        let edgeColor = 'rgba(88, 166, 255, 0.15)';
        let edgeWidth = 1;
...
async function init() {
    try {
        ...
        const response = await fetch('./viz-data.enc.json');
        const encryptedData = await response.text();
        const secretKey = 'thought-search-monochrome-key-2026';

        const binaryString = atob(encryptedData);
        const len = binaryString.length;
        const bytes = new Uint8Array(len);
        for (let i = 0; i < len; i++) {
            bytes[i] = binaryString.charCodeAt(i) ^ secretKey.charCodeAt(i % secretKey.length);
        }
        ...
        const nodeLineColor = 'rgba(255, 255, 255, 0.1)';
        ...
        const defaultEdgeColor = 'rgba(88, 166, 255, 0.15)';

        const edgeTrace = {
            x: originalEdgeX, y: originalEdgeY, z: originalEdgeZ,
            mode: 'lines', type: 'scatter3d',
            line: { color: defaultEdgeColor, width: 1 },
            hoverinfo: 'none'
        };
            ...
            searchInput.addEventListener('input', function () {
                ...
                searchTimeout = setTimeout(async () => {
                    try {
                        ...
                        globalNodes.forEach((node, nodeIdx) => {
                            const title = (node.metadata.title || node.metadata.filename || "").toLowerCase();
                            ...
                            let idx = content.indexOf(lowerQuery);
                            let matchCount = 0;
                            while (idx !== -1) {
                                matchCount++;
                                idx = content.indexOf(lowerQuery, idx + lowerQuery.length);
                            }
                            ...
                            if (score > 0) {
                                ...
                                const matchIdx = rawContent.toLowerCase().indexOf(lowerQuery);
                                if (matchIdx !== -1) {
                                    const start = Math.max(0, matchIdx - 40);
                                    const end = Math.min(rawContent.length, matchIdx + 80);
                                    snippet = (start > 0 ? "..." : "") + rawContent.substring(start, end).replace(/\n/g, " ") + (end < rawContent.length ? "..." : "");
                                } else {
                                    snippet = rawContent.substring(0, 100).replace(/\n/g, " ") + "...";
                                }
                        ...
                        searchResultsList.sort((a, b) => b.score - a.score);
                        const finalResults = searchResultsList.slice(0, 10);
